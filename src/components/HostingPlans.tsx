...
const HostingPlans = () => {
  ...
  return (
    <section dir="rtl" className="py-12 sm:py-24 bg-secondary/95">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl sm:text-5xl font-black text-primary mb-4 sm:mb-6 text-center drop-shadow-lg">
          خطط الاستضافة
        </h2>
        <p className="text-xl sm:text-2xl font-bold text-gray-200 mb-8 sm:mb-16 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
          {plans.map((plan, index) => (
            <div 
              key={index} 
              className="bg-white/10 backdrop-blur-lg rounded-xl p-4 sm:p-8 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-primary/20"
            >
              <div className="text-center">
                <h3 className="text-2xl sm:text-3xl font-bold text-primary mb-2 sm:mb-4">{plan.name}</h3>
                <p className="text-lg sm:text-xl text-gray-300 mb-4 sm:mb-6">{plan.description}</p>
                <div className="text-4xl sm:text-5xl font-black text-white mb-6 sm:mb-8 drop-shadow-lg">
                  {plan.price}
                  <span className="text-xl sm:text-2xl text-gray-300"> ج.م</span>
                </div>
                <ul className="space-y-4 sm:space-y-6 mb-6 sm:mb-8 text-right">
                  {plan.features.map((feature, fIndex) => (
                    <li key={fIndex} className="flex items-center gap-2 sm:gap-4">
                      <feature.icon className="w-5 h-5 sm:w-6 sm:h-6 text-primary shrink-0" />
                      <span className="text-base sm:text-lg text-gray-200">{feature.text}</span>
                    </li>
                  ))}
                </ul>
                <Button 
                  onClick={handleSubscribe}
                  className="w-full bg-primary hover:bg-primary-dark text-white text-lg sm:text-xl font-bold py-4 sm:py-6
